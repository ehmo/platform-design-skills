...
description: Apple Human Interface Guidelines for iPhone. Validates spacing guidelines, checks Dynamic Type support, implements safe area layouts, and audits HIG compliance. Use when building, reviewing, or refactoring iPhone interfaces with SwiftUI or UIKit. Triggers on tasks involving iOS, SwiftUI, UIKit, iPhone UI, Dynamic Type, Dark Mode, or HIG compliance.
license: MIT
metadata:
...
Use a tab bar instead. If you have more than 5 sections, consolidate or use a "More" tab.
...
Always pair color with text, icons, or shapes to convey meaning.
...
Follow these steps when building or auditing an iPhone interface:

1. **Layout foundations**: Configure safe areas with `.ignoresSafeArea` only for backgrounds. Set all interactive targets to minimum 44pt. Use the 8pt grid for spacing.
2. **Navigation structure**: Choose `TabView` (3-5 top-level sections) or `NavigationStack` (hierarchical). Verify large titles on root views, inline on children.
3. **Dynamic Type**: Apply `.font(.body)` and system text styles throughout. Test at all accessibility sizes. Verify no text truncation at AX5.
4. **Dark Mode**: Use semantic colors (`Color(.systemBackground)`, `Color(.label)`). Test both appearances. Verify contrast ratios meet 4.5:1 (text) and 3:1 (UI components).
5. **Accessibility audit**: Enable VoiceOver, navigate every screen. Verify all images have meaningful labels, all controls are reachable, focus order is logical.
6. **Gestures & input**: Confirm standard gesture behaviors (swipe-back, pull-to-refresh). Verify no gesture conflicts with system gestures.
7. **Final checklist**: Run through the evaluation checklist in [REFERENCE.md](./REFERENCE.md).

---

## Quick Reference & Anti-Patterns

See [REFERENCE.md](./REFERENCE.md) for the component quick reference table, full HIG compliance checklist, and anti-patterns list.
